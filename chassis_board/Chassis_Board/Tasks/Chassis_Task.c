...
static void Set_Chassis_VxVy(fp32 yaw_chassis_zero_rad, fp32 *chassis_vx, fp32 *chassis_vy)
{
	...
	else // 导航模式（进入Set_FollowGimbal_VxVy函数时不是遥控器控制模式就是导航模式，所以不用再判断一次是否为导航模式）
	{
		if (!toe_is_error(NAV_TOE))
		{
			...
			if(nav_ctrl.rmul_first_go_to_middle && !nav_ctrl.at_middle_section)
				ramp_coeff = 0.8f;
			else if(nav_ctrl.updownhill_state == 2)
				ramp_coeff = 0.08f;
			else
...
static void Set_Rotate_Wz(fp32 *wz)
{
	...
	else																	  // 导航模式下的小陀螺速度设置
	{
		...
		else
		{
			...
			{
				fp32 nav_compete_middle_wz_coeff = limit(nav_speed_rpm_now / NAV_MAX_SPEED, 0.0f, 1.0f);
				*wz = ROTATE_WZ_LOW * RAD_PER_SEC_TO_RPM + (1 - nav_compete_middle_wz_coeff) * (ROTATE_WZ_HIGH * RAD_PER_SEC_TO_RPM - ROTATE_WZ_LOW * RAD_PER_SEC_TO_RPM);
			}
			else if (nav_ctrl.health_state == HEALTH_HURT) // 被弹丸打了
			{
				// 根据底盘xy轴速度设置逃跑陀螺速度，速度越快陀螺转得越慢，保证在一定功率上限下的逃脱机率变高
				fp32 nav_escape_wz_coeff = limit(nav_speed_rpm_now / NAV_MAX_SPEED, 0.0f, 1.0f);
				*wz = ROTATE_WZ_HIGH * RAD_PER_SEC_TO_RPM + (1 - nav_escape_wz_coeff) * (ROTATE_WZ_MAX * RAD_PER_SEC_TO_RPM - ROTATE_WZ_HIGH * RAD_PER_SEC_TO_RPM);
			}
			else if (nav_ctrl.at_middle_section) // 在中央增益区，高转速转
			{
				*wz = ROTATE_WZ_HIGH * RAD_PER_SEC_TO_RPM;
			}
			else
				*wz = ramp_control(*wz, -ROTATE_WZ_MEDIUM * RAD_PER_SEC_TO_RPM, 0.8f);
			//			*wz = ramp_control(*wz, -ROTATE_WZ_MIN * RAD_PER_SEC_TO_RPM, 0.8f);
		}
	}
}
