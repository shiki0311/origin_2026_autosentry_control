...
static void Chassis_Mode_Update(chassis_mode_t *mode)
{
	...
	static bool_t bumpy_force_safe = FALSE;

	if (nav_ctrl.just_revive == 1 && last_just_revive == 0) //死在颠簸路段上，复活的时候判断一下有没有能力脱困，不行就等超电充会电
	{
		if (chassis_control.chassis_max_power_mode == PASS_BUMPY && nav_ctrl.referee_power_limit < 70 && cap_data.cap_per < (CAP_USE_THRESHOLD + 0.4f))
		{
			bumpy_force_safe = TRUE;
		}
	}
	...
	if (rc_ctrl_safe || bumpy_force_safe)
	{
