...
void CAN1_Timer_Callback(TimerHandle_t xTimer)
{
    ...
    if (xQueueReceive(CAN1_send_queue, &SendCanTxMsg, 0)) // 接收队列信息,禁止等待！！！
    {
        ...
        if (HAL_CAN_AddTxMessage(&hcan1, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0) != HAL_OK)
        {
            if (HAL_CAN_AddTxMessage(&hcan1, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0) != HAL_OK)
            {
                HAL_CAN_AddTxMessage(&hcan1, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0);
            }
        }
    }
}
...
void CAN2_Timer_Callback(TimerHandle_t xTimer)
{
    ...
    {
        // 寻空邮箱发送数据，发送失败继续尝试发送，最多发三次
        if (HAL_CAN_AddTxMessage(&hcan2, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0) != HAL_OK)
        {
            if (HAL_CAN_AddTxMessage(&hcan2, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0) != HAL_OK)
            {
                HAL_CAN_AddTxMessage(&hcan2, &SendCanTxMsg.tx_header, SendCanTxMsg.data, (uint32_t *)CAN_TX_MAILBOX0);
            }
        }
    }
}
