...
/*******************************CAN发送时的外设实例映射********************************/
#define RC_TO_CHASSIS_FIRST_CAN hcan2
#define RC_TO_CHASSIS_SECOND_CAN hcan2
#define BIG_YAW_DM6006_TransCAN hcan2

#define SMALL_YAW_AND_PITCH_TransCAN hcan1
#define FRIC_M3508_TransCAN hcan1
#define DIAL_TransCAN hcan1
/*******************************CAN发送相关宏定义********************************/
...
#define RC_TO_CHASSIS_FIRST_RESEND_QUEUE CAN2_resend_queue
#define RC_TO_CHASSIS_SECOND_RESEND_QUEUE CAN2_resend_queue
#define BIG_YAW_DM6006_RESEND_QUEUE CAN2_resend_queue
#define SMALL_YAW_AND_PITCH_RESEND_QUEUE CAN1_resend_queue
#define FRIC_M3508_RESEND_QUEUE CAN1_resend_queue
#define DIAL_RESEND_QUEUE CAN1_resend_queue
/*********************************************CAN发送缓冲区*********************************************************************/
CanTxMsgTypeDef rc_to_chassis_first_send_buffer;  // 传rc_ctrl.rc.ch数组的前四个元素的can报文全局缓冲区
CanTxMsgTypeDef rc_to_chassis_second_send_buffer; // 传rc_ctrl.rc.ch数组的第五个元素和rc_ctrl.rc.s数组的can报文全局缓冲区
CanTxMsgTypeDef big_yaw_send_buffer;			  // 大yaw轴达妙6006 can报文全局缓冲区
CanTxMsgTypeDef small_yaw_and_pitch_send_buffer;  // pitch轴，小yaw轴6020 can报文全局缓冲区
CanTxMsgTypeDef fric_send_buffer;				  // 摩擦轮3508can报文全局缓冲区
CanTxMsgTypeDef dial_send_buffer;				  // 拨弹盘can报文全局缓冲区
...
void Can_Buffer_Init(void)
{
	...
	struct
	{
		...
		{&rc_to_chassis_first_send_buffer, RC_TO_CHASSIS_FIRST_ID},
		{&rc_to_chassis_second_send_buffer, RC_TO_CHASSIS_SECOND_ID},
		{&big_yaw_send_buffer, BIG_YAW_DM6006_TransID},
		{&small_yaw_and_pitch_send_buffer, SMALL_YAW_AND_PITCH_TransID},
...
void Allocate_Can_Buffer(int16_t data1, int16_t data2, int16_t data3, int16_t data4, CAN_CMD_ID can_cmd_id)
{
	...
	case CAN_RC_TO_CHASSIS_FIRST_CMD:
		pTxMsg = &rc_to_chassis_first_send_buffer;
		break;
	case CAN_RC_TO_CHASSIS_SECOND_CMD:
		pTxMsg = &rc_to_chassis_second_send_buffer;
		break;
	case CAN_BIG_YAW_CMD:
		pTxMsg = &big_yaw_send_buffer;
		break;
	case CAN_SMALL_YAW_AND_PITCH_CMD:
		pTxMsg = &small_yaw_and_pitch_send_buffer;
		break;
	case CAN_FRIC_CMD:
		pTxMsg = &fric_send_buffer;
		break;
	case CAN_DIAL_CMD:
		pTxMsg = &dial_send_buffer;
		break;
	default:
		return; // 无效命令，直接返回
	}
...
void Ctrl_DM_Motor(float _pos, float _vel, float _KP, float _KD, float _torq) // can2
{
	...
	Allocate_Can_Buffer(data1, data2, data3, data4, CAN_BIG_YAW_CMD);
}
...
void CAN_TX_TimerIRQHandler()
{
	...
	{
		xQueueSendFromISR(SMALL_YAW_AND_PITCH_RESEND_QUEUE, &small_yaw_and_pitch_send_buffer, &xHigherPriorityTaskWoken); // 如果发送失败送入队列等待重新发送
	}
	if (HAL_CAN_AddTxMessage(&FRIC_M3508_TransCAN, &fric_send_buffer.tx_header, fric_send_buffer.data, &send_mail_box) != HAL_OK)
	{
		xQueueSendFromISR(FRIC_M3508_RESEND_QUEUE, &fric_send_buffer, &xHigherPriorityTaskWoken); // 如果发送失败送入队列等待重新发送
	}
	if (HAL_CAN_AddTxMessage(&DIAL_TransCAN, &dial_send_buffer.tx_header, dial_send_buffer.data, &send_mail_box) != HAL_OK)
	{
