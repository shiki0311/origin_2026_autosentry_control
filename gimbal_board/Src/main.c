...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
// 信号量句柄，用于INS和Gimbal任务同步
SemaphoreHandle_t ins_init_done_semaphore;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  Can_Filter_Init();
  Can_Msg_Init();
  imu_can_init(BIG_YAW_DMIMU_TransID, BIG_YAW_DMIMU_RecID, &hcan2);
  /******************USART相关初始化***********************/
  remote_control_init();                                                                  // 使能usart3空闲中断并开启接收DMA，准备接收遥控器数据
